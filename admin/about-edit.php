<?php
	...
	$admin_user->setActivePageGrp("about");
	...
	if(Input::exists())
	{
		if(Input::hasValue('submit-edit'))
			$admin_user->call("admin/db-about", "about_add");
		if(Input::hasValue('submit-delete-confirm'))
			$admin_user->call("admin/db-about", "about_delete");

		if($admin_user->successfullyCalled('about_add'))
		{
			$success_edit = true;
			Input::clearType('post');
		}
		if($admin_user->successfullyCalled('about_delete'))
			$success_delete = true;
	}
	...
	if(Input::hasValue('item', 'get') && !$success_delete)
	{
		try
		{
			$statement = "	SELECT 	".config::get('table_prefix')."about.*
							FROM ".config::get('table_prefix')."about
							WHERE md5(".config::get('table_prefix')."about.id) = :id
							LIMIT 1;";
			...
			else
			{
				...
				Input::set('content', $data['description']);
				Input::set('img', $data['image']);
				// Input::set('id', $data['id']);
				// Input::set('title', $data['title']);
		...
		<div class="content-wrapper">
			<div class="page-section">
				<div class="col col-80 content-col">
					...
					<?php if($success_edit) echo '<div class="success">You have succesfully edited About Section!</div>'; ?>
					<?php if($success_delete) echo '<div class="success">You have succesfully deleted About Section!</div>'; ?>
					<?php
						if(Session::hasValue(Config::get("table_prefix").'features-add-success'))
						{
							echo '<div class="section-content">';
							echo 	'<div class="success">You have succesfully added a About Section!<br><a href="'.Config::get('admin_url').'features-add">Click here to add another.</a></div>';
							echo '</div>';
							...
							<div class="img-preview">  
								<?php
									...
									else
									{
										...
										echo '<img src="'.Config::get('absolute_url').'images/'.Input::get("img").'" alt="Item Image">';
					...
					<div class="search-listing">
						<?php
							include($base_url.'includes/searches/about.php');
